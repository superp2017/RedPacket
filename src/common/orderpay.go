...
func wx_pub_pay(order *ST_Order) (map[string]string, error) {
...
func direct_transfer(transfer *ST_Transfer) (map[string]string, error) {
	b := make([]byte, 8)
	rand.Read(b)
	nonce_str := strings.ToUpper(coder.EncodeToString(b))
	...
	params := make(wxpay.Params)
	// 查询企业付款接口请求参数//
	params.SetString("mch_appid", transfer.AppId)

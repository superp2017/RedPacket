...
//获取年月日时
func GetYMDH(time time.Time) string {
	return time.Format("2006-01-02 15")
}

func GetYM_CH(time time.Time) string {
	return time.Format("2006年01月")
}

//获取年月日
func GetYMD_CH(time time.Time) string {
	return time.Format("2006年01月02日")
}

//获取年月日时
func GetYMDH_CH(time time.Time) string {
	return time.Format("2006年01月02日 15时")
}
...
///转化任意形式的字符串时间
func ParseTimeFromString(date string, format string) (time.Time, error) {
